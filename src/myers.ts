// All credit goes to https://github.com/gliese1337/fast-myers-diff

// I'm not using the library so I can make a few minor changes.
...
export function* calcPatch<T, S extends Sliceable<T>>(
  ...
  const slice = ArrayBuffer.isView(xs)
    ...
    : xs.slice;
  for (const v of diff_core(0, xs.length, 0, ys.length, eq)) {
